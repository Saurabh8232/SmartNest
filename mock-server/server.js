...
let digitalBoard = {
  ...
  totalCurrent: 1.1,
  relays: [
    { id: "d1", name: "Smart Plug 1", isOn: true,  current: 0.6, power: 132, status: "normal",  switchState: "released" },
    { id: "d2", name: "Smart Plug 2", isOn: false, current: 0.0, power: 0,   status: "normal",  switchState: "released" },
    { id: "d3", name: "LED Strip",    isOn: true,  current: 0.5, power: 110, status: "normal",  switchState: "pressed"  },
  ],
};
...
let alerts = [
  { id: "a1", type: "electrical", code: "OV01", title: "High Voltage Detected", description: "Voltage exceeded 240V threshold.", severity: "warning", suggestedSolution: "Check main supply. If sustained, contact electrician.", timestamp: new Date(Date.now() - 300000).toISOString(), isResolved: false, deviceName: "Main Board" },
  { id: "a2", type: "communication", code: "CM01", title: "Digital Board Reconnected", description: "Lost connection for 30s, reconnected successfully.", severity: "info", suggestedSolution: "Monitor for recurring disconnections.", timestamp: new Date(Date.now() - 900000).toISOString(), isResolved: false, deviceName: "Digital Board" },
];
...
let voltage = 220.4, power = 462.0, current = 2.1, energy = 1.84;
...
function tick() {
  ...
  // Push live dashboard to all clients every 3 seconds
  io.emit("dashboard:update", getDashboardData());
}
setInterval(tick, 3000);

function getDashboardData() {
  const activeRelays = mainBoard.relays.filter(r => r.isOn).length;
  return {
    systemOnline: true,
...
io.on("connection", (socket) => {
  ...
  socket.on("main-board:relay-control", ({ relayId, action }) => {
    ...
    if (relay) { relay.isOn = action === "on"; relay.current = relay.isOn ? +(Math.random() * 2 + 0.3).toFixed(2) : 0; }
    mainBoard.totalCurrent = +mainBoard.relays.reduce((s, r) => s + r.current, 0).toFixed(2);
    io.emit("main-board:update", mainBoard);
    console.log(`Relay ${relayId} → ${action}`);
  });
  ...
  socket.on("digital-board:relay-control", ({ relayId, action }) => {
    ...
    if (relay) { relay.isOn = action === "on"; relay.current = relay.isOn ? +(Math.random() * 0.8 + 0.1).toFixed(2) : 0; relay.power = +(relay.current * 220).toFixed(0); }
  ...
  socket.on("ac:control", ({ action, value }) => {
    ...
    console.log(`AC command: ${action}`, value ?? "");
  });
